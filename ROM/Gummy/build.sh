...
make_command="gummy"
dropbox_path="$HOME/Dropbox/Android/gummy"
...
zipname=$(ls out/target/product/$device_codename/Gummy-*.zip | sed "s/out\/target\/product\/$device_codename\///")
if [[ $debug = 1 ]]; then
	echo '##########'
	echo 'zipname'
	echo '##########'
...
if [[ $release = 1 ]]; then		# upload the compiled build
	...
	echo '##########'
	if [[ $nightly = 1 ]]; then
#		scp ./out/target/product/$device_codename/$zipname goo.im:public_html/GummyROM/$zipname &	# upload via ssh too goo.im servers
		if [[ $mirror = 1 ]]; then
			cp ./out/target/product/$device_codename/$zipname $HOME/droideveloper/GummyROM/$zipname &
		fi
	else
#		scp ./out/target/product/$device_codename/$zipname goo.im:public_html/GummyROM/$zipname 	# upload via ssh too goo.im servers
		if [[ $mirror = 1 ]]; then
